...
export default function EducationCard({ education }: EducationCardProps) {
  const { institution, degree, description, startDate, endDate, location } = education;
    ...
    <article className="hover:bg-background-light transition-colors duration-300 rounded-3xl overflow-hidden p-4 group">
      <div className="flex flex-col md:flex-row md:gap-6">
        ...
        <div className="flex-1">
          <h3 className="text-2xl font-bold text-foreground mb-2">{degree}</h3>
          <p className="font-medium text-secondary mb-4 leading-relaxed">{institution}</p>
          {description && (
